...
st.markdown(
    """
    <style>
    .stApp { background-color: #0B3F19; color: #E2E8F0; }
...
# 1. 첫 번째 질문: 톨만 온도 인지 여부 (기본 선택을 빈 값으로 두기 위해 변경)
answer = st.radio(
...
if answer == "no":
    ...
    # 반지름 입력값이 0보다 커야(즉 입력해야) 최종 계산 결과가 뜸
    if rs > 0:
        st.write("---")
    ...
    elif answer == "yes" and rs == 0:
        # yes를 눌렀지만 아직 반지름을 입력하지 않은 상태
        st.info("💡 슈바르츠실트 반지름 값을 1 이상으로 입력하시면 계산 결과가 나타납니다.")